...
public:
  T2KNOvAFlatTree(nuiskey samplekey) {
    ...
    if (samplekey.Has("weight_friend_name")) {
      isWeightFriend = true;
      FriendName = samplekey.GetS("weight_friend_name");
    } else {
      isWeightFriend = false;
    }
  ...
  void InitTree() {
    ...
    eventVariables = new TTree(
        isWeightFriend ? (std::string("T2KNOvATruthTree_") + FriendName).c_str()
                       : "T2KNOvATruthTree",
        "");
    eventVariables->SetDirectory(odir);

    eventVariables->Branch(
        isWeightFriend ? FriendName.c_str() : "RWWeight", &RWWeight,
        isWeightFriend ? (FriendName + "/D").c_str() : "RWWeight/D");
