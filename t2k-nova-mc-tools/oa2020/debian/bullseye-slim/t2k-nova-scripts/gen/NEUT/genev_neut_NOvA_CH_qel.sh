...
if [ -e NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root ]; then
   echo "Already have file: NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root, not overwriting."
   exit 1
fi
...
echo "Running neutroot2 NOvAND.CH.qel.${BEAMMODE}.card NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root for ${NEVS} events."
neutroot2 NOvAND.CH.qel.${BEAMMODE}.card NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root &> /dev/null

if [ -e NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root ]; then
   rm -f fort.77 NOvAND.CH.qel.${BEAMMODE}.card

   PrepareNEUT -i NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root \
               -f ${FLUX_FILE},${FLUX_HIST} -G
else
   echo "Failed to produce expected output file: NOvAND.CH.qel.${BEAMMODE}.${RUNNUM}.neut.root"
   exit 1
fi
