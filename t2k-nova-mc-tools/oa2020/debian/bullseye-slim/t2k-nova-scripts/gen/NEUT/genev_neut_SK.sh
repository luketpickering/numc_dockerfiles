...
if [ -e SK.${BEAMMODE}.${RUNNUM}.neut.root ]; then
   echo "Already have file: SK.${BEAMMODE}.${RUNNUM}.neut.root, not overwriting."
   exit 1
fi
...
echo "Running neutroot2 SK.${BEAMMODE}.card SK.${BEAMMODE}.${RUNNUM}.neut.root for ${NEVS} events."
neutroot2 SK.${BEAMMODE}.card SK.${BEAMMODE}.${RUNNUM}.neut.root &> /dev/null

if [ -e SK.${BEAMMODE}.${RUNNUM}.neut.root ]; then
   rm -f fort.77 SK.${BEAMMODE}.card

   PrepareNEUT -i SK.${BEAMMODE}.${RUNNUM}.neut.root \
               -f ${FLUX_FILE},${FLUX_HIST} -G
else
   echo "Failed to produce expected output file: SK.${BEAMMODE}.${RUNNUM}.neut.root"
   exit 1
fi
