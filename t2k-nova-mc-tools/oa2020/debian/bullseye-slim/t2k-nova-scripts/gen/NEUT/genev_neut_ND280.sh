...
if [ -e ND280.${BEAMMODE}.${RUNNUM}.neut.root ]; then
   echo "Already have file: ND280.${BEAMMODE}.${RUNNUM}.neut.root, not overwriting."
   exit 1
fi
...
echo "Running neutroot2 ND280.${BEAMMODE}.card ND280.${BEAMMODE}.${RUNNUM}.neut.root for ${NEVS} events."
neutroot2 ND280.${BEAMMODE}.card ND280.${BEAMMODE}.${RUNNUM}.neut.root &> /dev/null

if [ -e ND280.${BEAMMODE}.${RUNNUM}.neut.root ]; then
   rm -f fort.77 ND280.${BEAMMODE}.card

   PrepareNEUT -i ND280.${BEAMMODE}.${RUNNUM}.neut.root \
               -f ${FLUX_FILE},${FLUX_HIST} -G
else
   echo "Failed to produce expected output file: ND280.${BEAMMODE}.${RUNNUM}.neut.root"
   exit 1
fi
